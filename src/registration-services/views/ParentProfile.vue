...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      const data = await ParentService.getParent();
      parent.value = data.length > 0 ? data[0] : null;
      editable.value = { ...parent.value };
    });
...
<template>
  <pv-card v-if="parent">
    ...
    <template #content>
      ...
      <form class="profile-form">
        <div v-for="field in ['name', 'address', 'phone', 'childrenCount', 'preferences', 'city']" :key="field" class="form-group">
          <label>{{ field.charAt(0).toUpperCase() + field.slice(1) }}</label>
          <div class="input-icon-group">
            <input
                :type="field === 'childrenCount' ? 'number' : 'text'"
  ...
  <pv-card v-if="showNewCard">
    ...
    <template #content>
      ...
      <p><strong>Children Count:</strong> {{ parent.childrenCount }}</p>
      <p><strong>Preferences:</strong> {{ parent.preferences }}</p>
